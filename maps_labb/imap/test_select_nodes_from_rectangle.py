...
from mapvis.extractor import extract_osm_nodes
from mapvis.extractor import select_nodes_in_rectangle
...
class TestSelectNodes(unittest.TestCase):
    ...
    def test_some_nodes_inside(self):
        ...
        selected_nodes = select_nodes_in_rectangle(nodes, -10, 10, -10, 10)
        self.assertTrue(any(x in selected_nodes.get_nodes().keys() for x in nodes.get_nodes().keys()))
    ...
    def test_all_nodes_inside(self):
        ...
        nodes.add(2, -5, -5)
        selected_nodes = select_nodes_in_rectangle(nodes, -10, 10, -10, 10)
        self.assertTrue(nodes.get_nodes().keys() == selected_nodes.get_nodes().keys())
